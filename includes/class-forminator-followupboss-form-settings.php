...
class Forminator_Followupboss_Form_Settings extends Forminator_Integration_Form_Settings {
	...
	private function destination_fields() {
		return array(
			'email'      => esc_html__( 'Email Address', 'forminator-followup-boss' ),
			'first_name' => esc_html__( 'First Name', 'forminator-followup-boss' ),
			'last_name'  => esc_html__( 'Last Name', 'forminator-followup-boss' ),
			'phone'      => esc_html__( 'Phone', 'forminator-followup-boss' ),
		);
	}
	...
	public function pick_name( $submitted_data ) {
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $name ) ) {
					throw new Forminator_Integration_Exception( esc_html__( 'Please enter a name for this connection.', 'forminator-followup-boss' ) );
				}
	...
	public function map_fields( $submitted_data ) {
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( empty( $fields_map['email'] ) && empty( $fields_map['phone'] ) ) {
					$input_exceptions->add_input_exception( esc_html__( 'Map at least an Email or Phone field.', 'forminator-followup-boss' ), 'email_error' );
				}
				foreach ( $fields as $key => $title ) {
					if ( ! empty( $fields_map[ $key ] ) && ! in_array( $fields_map[ $key ], $forminator_field_element_ids, true ) ) {
						$input_exceptions->add_input_exception(
							sprintf( /* translators: %s: field title. */ esc_html__( 'Please assign a valid field for %s', 'forminator-followup-boss' ), esc_html( $title ) ),
							$key . '_error'
						);
					}
				}
		...
		$buttons = array(
			'disconnect' => array(
				'markup' => Forminator_Integration::get_button_markup( esc_html__( 'Deactivate', 'forminator' ), 'sui-button-ghost forminator-addon-form-disconnect' ),
			),
			'submit'     => array(
				'markup' => '<div class="sui-actions-right">' .
					Forminator_Integration::get_button_markup( esc_html__( 'Save', 'forminator' ), 'forminator-addon-finish' ) .
					'</div>',
			),
		);
