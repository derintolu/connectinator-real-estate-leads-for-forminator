...
final class Forminator_Followupboss extends Forminator_Integration {
	...
	public function __construct() {
		$this->_description    = esc_html__( 'Send Forminator leads to Follow Up Boss.', 'real-estate-leads-for-forminator' );
	...
	public function setup_api( $submitted_data, $form_id = 0 ) {
		...
		if ( $is_submit ) {
			try {
				...
				if ( empty( $api_key ) ) {
					throw new Forminator_Integration_Exception( esc_html__( 'Please enter your Follow Up Boss API key.', 'real-estate-leads-for-forminator' ) );
				}
		...
		if ( $show_success ) {
			...
			if ( $this->is_connected() ) {
				$buttons['disconnect'] = array(
					'markup' => self::get_button_markup( esc_html__( 'Disconnect', 'real-estate-leads-for-forminator' ), 'sui-button-ghost forminator-addon-disconnect' ),
				);
			}
			$buttons['submit'] = array(
				'markup' => '<div class="sui-actions-right">' .
					self::get_button_markup( esc_html__( 'Connect', 'real-estate-leads-for-forminator' ), 'forminator-addon-connect' ) .
					'</div>',
			);
	...
	public function validate_api( $api_key ) {
		...
		if ( empty( $identity['account'] ) ) {
			throw new Forminator_Integration_Exception( esc_html__( 'Could not verify the Follow Up Boss account for this key.', 'real-estate-leads-for-forminator' ) );
		}
	}
